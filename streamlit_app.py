...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption("🧸 Junk Toys v4.10 — Ranking completo con detalles")

tab1, tab2, tab3, tab4 = st.tabs(["📊 Señales en Vivo", "📈 Backtesting", "📉 Métricas", "🏆 Top 10 Long/Short"])

# ============================================================
# TAB 1: SEÑALES EN VIVO (con padding y detalles de todos los aprobados)
# ============================================================
with tab1:
    ...
    with st.spinner("🔍 Escaneando el mercado..."):
        try:
            de = DataEngine()
            # Aumentamos el número de pares para cubrir todo Binance
            symbols = de.get_usdt_pairs(min_volume_usd=200000, max_pairs=200)
            st.info(f"📊 Escaneando {len(symbols)} activos con volumen > $200,000")

            data = {}
            using_sample = False
            # Tomamos los primeros 50 para no saturar (ajustable)
            ...
            # ===== DETALLES DE TODAS LAS SEÑALES APROBADAS =====
            st.markdown("---")
            st.subheader("📋 Detalles completos de TODAS las señales aprobadas")
            approved_signals = [s for s in all_signals if s.is_valid]
            if approved_signals:
                # Ordenar por confianza
                approved_signals.sort(key=lambda x: x.confidence, reverse=True)
                for s in approved_signals:
                    with st.expander(f"✅ {s.symbol} ({s.direction}) — Confianza {s.confidence*100:.1f}%"):
                        st.json({
                            "Activo": s.symbol,
...
# ============================================================
# TAB 2: BACKTESTING (con más pares)
# ============================================================
with tab2:
    ...
    if run_backtest_btn:
        with st.spinner("🔄 Descargando datos históricos y ejecutando simulación..."):
            try:
                de = DataEngine()
                # Obtener muchos pares para el backtest
                symbols = de.get_usdt_pairs(min_volume_usd=200000, max_pairs=200)
                if not symbols:
                    symbols = ['BTC/USDT', 'ETH/USDT', 'BNB/USDT', 'SOL/USDT', 'XRP/USDT']
                # Tomar los primeros 20 para no demorar demasiado (ajustable)
                symbols = symbols[:20]
                data = {}
...
# ============================================================
# TAB 4: TOP 10 DETALLADO (con padding)
# ============================================================
with tab4:
...
st.markdown("---")
st.caption("🧸 Junk Toys Band Project v4.10 — Ranking siempre visible, detalles de todos los aprobados y backtest ampliado. 🧸🐻🎉")
